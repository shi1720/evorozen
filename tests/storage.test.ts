...
it('persists committed data across local restarts and rolls back failed transactions', async () => {
  ...
  try {
    ...
    await db.query("INSERT INTO users(id,email,name,workspace_name,currency,password_hash,recovery_hash) VALUES('persistent-owner','persistence@example.com','Owner','Cafe','USD','unused-hash','unused-hash')");
    await expect(db.transaction(async (tx) => {
      await tx.query("UPDATE users SET workspace_name='Incomplete update' WHERE id='persistent-owner'");
      throw new Error('Simulated interrupted operation');
    })).rejects.toThrow('Simulated interrupted operation');
    await db.close();
    db = await createDatabase({ dataDir: directory });
    const row = (await db.query<{ workspace_name: string }>("SELECT workspace_name FROM users WHERE id='persistent-owner'")).rows[0];
...
it('atomically enforces shared and workspace AI budgets across concurrent requests', async () => {
  ...
  try {
    const attempts = await Promise.allSettled(['first', 'first', 'first', 'second', 'second'].map((id) => reserveAiCall(db, id, { global: 3, user: 2 })));
    ...
    for (const result of attempts) if (result.status === 'rejected') expect(result.reason.code).toBe('AI_DAILY_BUDGET');
    expect((await db.query<{ hits: number }>('SELECT hits FROM rate_limits ORDER BY hits DESC')).rows.map((row) => row.hits)).toEqual([3, 2, 1]);
