...
export const isFinalized = (value: RecoveryCase) =>
  ['approved', 'sent', 'partial', 'resolved'].includes(value.status);
...
export function checkVersion(value: RecoveryCase, version: number) {
  if (value.version !== version)
    throw new ApiError(
      409,
      'This case changed in another tab. Refresh the case and try again.',
      'VERSION_CONFLICT',
    );
}
export function checkNotAnalyzing(row: CaseRow) {
  if (
    ...
    Date.now() - new Date(row.analysis_started_at).getTime() < 180_000
  ) {
    throw new ApiError(
      409,
      'Analysis is already running for this case. Please wait for it to finish.',
...
export async function caseRow(
  ...
  const result = await tx.query<CaseRow>(
    `SELECT data,version,analysis_token,analysis_started_at FROM recovery_cases WHERE id=$1 AND user_id=$2${lock ? ' FOR UPDATE' : ''}`,
    [caseId, userId],
  );
...
export async function saveCase(
  ...
  value.updatedAt = now();
  await tx.query(
    'UPDATE recovery_cases SET data=$1, version=$2, updated_at=$3 WHERE id=$4 AND user_id=$5',
    [JSON.stringify(value), value.version, value.updatedAt, value.id, userId],
  );
  return value;
}
export async function addActivity(
  ...
  await tx.query(
    'INSERT INTO activities(id,user_id,case_id,case_title,action,detail) VALUES($1,$2,$3,$4,$5,$6)',
    [randomUUID(), userId, value?.id ?? null, value?.title ?? null, action, detail],
  );
}
export async function listCases(
  ...
  const projection = compact
    ...
    : 'data';
  const result = await tx.query<{ data: RecoveryCase }>(
    `SELECT ${projection} AS data FROM recovery_cases WHERE user_id=$1 ORDER BY updated_at DESC`,
    [userId],
  );
  return result.rows.map((row) => row.data);
}
export async function listSuppliers(tx: Queryable, userId: string): Promise<Supplier[]> {
  const result = await tx.query<{ data: Supplier }>(
    'SELECT data FROM suppliers WHERE user_id=$1 ORDER BY normalized_name',
    [userId],
  );
  return result.rows.map((row) => row.data);
}
export async function listActivities(
  ...
  if (caseId) {
    params.push(caseId);
    filter = ` AND case_id=$${params.length}`;
  }
  let bound = '';
  if (limit !== null) {
    params.push(limit);
  ...
  const result = await tx.query<{
    ...
    `SELECT id,case_id,case_title,action,detail,created_at FROM activities WHERE user_id=$1${filter} ORDER BY created_at DESC,id DESC${bound}`,
    params,
  );
  ...
  return (
    suppliers.find(
      (supplier) =>
        ...
        supplier.aliases.some((alias) => normalizeName(alias) === normalizeName(name)),
    ) ?? null
  );
}
export async function ensureSupplier(
  ...
  const result = await tx.query<{ data: Supplier }>(
    'INSERT INTO suppliers(id,user_id,data,normalized_name) VALUES($1,$2,$3,$4) ON CONFLICT(user_id,normalized_name) DO UPDATE SET normalized_name=EXCLUDED.normalized_name RETURNING data',
    [supplier.id, userId, JSON.stringify(supplier), normalizeName(name)],
  );
...
export async function insertCase(tx: Queryable, userId: string, value: RecoveryCase) {
  await tx.query('INSERT INTO recovery_cases(id,user_id,data,version) VALUES($1,$2,$3,$4)', [
    value.id,
    userId,
