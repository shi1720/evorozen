...
const quantitySchema = z
  ...
  .min(0)
  .max(100_000)
  .refine((n) => Math.abs(n * 1000 - Math.round(n * 1000)) < 1e-7);
const extractionSchema = z
  .object({
    supplierName: z.string().min(1).max(200),
    invoiceReference: z.string().min(1).max(100),
      ...
      .array(
        z
          .object({
            ...
            kind: z.enum(['shortage', 'damage', 'price_difference', 'unmatched']),
            invoicedQuantity: quantitySchema,
            receivedQuantity: quantitySchema,
            unitPriceCents: z.number().int().positive().max(MAX_MONEY),
            unit: z.string().min(1).max(40),
            explanation: z.string().max(1000),
            confidence: z.enum(['high', 'medium', 'low']),
            invoiceEvidence: quoteSchema,
            deliveryEvidence: quoteSchema,
          })
          .strict(),
      )
      .max(80),
      ...
      .array(
        z
          .object({
            ...
            invoiceReference: z.string().min(1).max(100),
            currency: z.enum(['USD', 'INR', 'GBP', 'EUR']),
            amountCents: z.number().int().positive().max(MAX_MONEY),
            evidence: z.array(quoteSchema).min(1).max(8),
          })
          .strict(),
      )
      .max(12),
...
function sourceSupplierHeader(
  text: string,
  supplierNames: string[],
): { name: string; quote: string } | null {
  const names = new Set(
    supplierNames
      .filter((value) => value.trim().length >= 3 && !/[=→↔]/.test(value))
      .map(normalized),
  );
  for (const line of text.split('\n').slice(0, 12)) {
    const name = line
      .trim()
      .replace(/^(?:supplier|vendor|issued by|from)\s*:\s*/i, '')
      .trim();
...
function hasReference(text: string, reference: string): boolean {
  return (
    !!reference.trim() &&
    new RegExp(`(?<![a-z0-9])${escapeRegExp(reference.trim())}(?![a-z0-9])`, 'i').test(text)
  );
}
function hasInvoiceReference(text: string, reference: string): boolean {
  return new RegExp(
    `(?:for\\s+)?invoice(?:\\s*(?:no\\.?|number|reference))?\\s*[:#-]?\\s*${escapeRegExp(reference)}(?![a-z0-9-])`,
    'i',
  ).test(text);
}
function hasCreditReference(text: string, reference: string): boolean {
  return new RegExp(
    `credit\\s*(?:note|memo)(?:\\s*(?:no\\.?|number|reference))?\\s*[:#-]?\\s*${escapeRegExp(reference)}(?![a-z0-9-])`,
    'i',
  ).test(text);
}
function hasInjection(text: string): boolean {
  return /ignore\s+(?:all\s+)?(?:previous|prior|above|system)\s+(?:instructions|prompts)|(?:reveal|print|exfiltrate)\s+(?:the\s+)?(?:system prompt|api key|secret)|(?:<\/?(?:system|assistant)>|\[INST\])|(?:override|disregard)\s+(?:your|the)\s+(?:rules|instructions)/i.test(
    text,
  );
}
function currencyIn(text: string): Currency[] {
  const currencies = (['USD', 'INR', 'GBP', 'EUR'] as Currency[]).filter((code) =>
...
function validateCurrency(text: string, currency: Currency): boolean {
  const declared = [...text.matchAll(/\b(?:currency|currency\s+code)\s*[:=]\s*([a-z]{3})\b/gi)].map(
    (match) => match[1].toUpperCase(),
  );
  if (declared.some((code) => code !== currency)) return false;
  const found = currencyIn(text);
  return found.length === 1 && found[0] === currency;
}
export function analysisSourceHash(
  documents: EvidenceDocument[],
  supplier?: Supplier | null,
): string {
  return hash(
    JSON.stringify({
      ...
      documents: documents
        .map((d) => [d.id, d.kind, hash(d.text)])
        .sort((a, b) => a[0].localeCompare(b[0])),
      supplier: supplier ? [supplier.id, supplier.name, supplier.aliases, supplier.notes] : null,
    }),
  );
}
...
export function multiplyCents(quantity: number, unitPriceCents: number): number {
  ...
  if (amount > MAX_MONEY)
    throw new EngineError('This line exceeds the supported recovery amount.', 'INVALID_MONEY');
  return amount;
}
...
function quantitySupported(quote: string, quantity: number, unit: string): boolean {
  ...
  return new RegExp(
    `(?:quantity|qty|received|delivered|invoiced|billed)\\s*[:=]\\s*${n}\\s*(?:${u}s?)(?![a-z])`,
    'i',
  ).test(quote);
}
function moneySupported(quote: string, cents: number): boolean {
  ...
  const intValue = Number.isInteger(cents / 100)
    ? `|${escapeRegExp(String(cents / 100))}(?![\\d.,])`
    : '';
  return new RegExp(
    `(?:USD|INR|GBP|EUR|[$£€₹])\\s*(?:${value}${intValue})(?!\\d)|(?:${value})\\s*(?:USD|INR|GBP|EUR)(?![a-z])`,
    'i',
  ).test(clean);
}
function labeledMoneySupported(quote: string, cents: number, kind: 'price' | 'credit'): boolean {
  const label =
    kind === 'price'
      ? /(?:unit\s*(?:price|cost)|price\s*(?:per|\/)\s*\w+)\s*[:=]?\s*([^|\n]+)/gi
      : /(?:credit\s*(?:total|amount)|total\s*credit|^total)\s*[:=]?\s*([^|\n]+)/gim;
  return [...quote.matchAll(label)].some((match) => {
    const amount = match[1].match(
      /^\s*((?:USD|INR|GBP|EUR|[$£€₹])\s*\d[\d,]*(?:\.\d{1,2})?|\d[\d,]*(?:\.\d{1,2})?\s*(?:USD|INR|GBP|EUR))(?=\s|$|[;)])/i,
    )?.[1];
    return !!amount && decimalMoney(amount) === cents;
  });
}
...
export function observedProductLabel(quote: string, document?: EvidenceDocument): string {
  const cells = tableCells(quote);
  if (document && cells.length > 1 && document.text.includes(quote)) {
    const rowIndex = document.text.slice(0, document.text.indexOf(quote)).split('\n').length - 1;
    const lines = document.text.split('\n');
    for (let index = rowIndex - 1; index >= Math.max(0, rowIndex - 12); index--) {
      const header = tableCells(lines[index]);
      if (header.length !== cells.length) continue;
      const productIndex = header.findIndex((value) =>
        ['item', 'product', 'description', 'sku', 'productdescription'].includes(normalized(value)),
      );
      if (productIndex >= 0) return cells[productIndex];
    }
  }
  return cells[0]?.replace(/\s+(?:received|delivered|quantity|qty)\s*:.*/i, '').trim() ?? '';
}
function decimalMoney(text: string): number | null {
  const number = text
    .replace(/\b(?:USD|GBP|EUR|INR)\b|[$£€₹]/gi, '')
    .replace(/\s/g, '')
    .replace(/,(?=\d{3}(?:\D|$))/g, '');
...
function tableFacts(
  ...
  quote: string,
  unit: string,
): { quantity: number; price: number | null; header: string } | null {
  ...
  for (let rowIndex = first; rowIndex <= last; rowIndex++) {
    ...
    for (let headIndex = rowIndex - 1; headIndex >= Math.max(0, rowIndex - 12); headIndex--) {
      ...
      const quantityIndex = names.findIndex((value) =>
        [
          'qty',
          'quantity',
          'received',
      ...
      const productIndex = names.findIndex((value) =>
        ['item', 'product', 'description', 'sku', 'productdescription'].includes(value),
      );
      const priceIndex = names.findIndex((value) =>
        ['unitprice', 'unitcost', 'price'].includes(value),
      );
      ...
      if (
        !quantity ||
        !recordedUnit ||
        normalized(recordedUnit).replace(/s$/, '') !== normalized(unit).replace(/s$/, '')
      )
        continue;
      return {
        quantity: Number(quantity[1]),
        price: priceIndex >= 0 ? decimalMoney(cells[priceIndex]) : null,
        header: lines[headIndex],
      };
    }
  }
  return null;
}
function financialEvidenceSupported(
  item: Pick<Finding, 'invoicedQuantity' | 'receivedQuantity' | 'unitPriceCents' | 'unit'>,
  invoiceEvidence: Citation,
  deliveryEvidence: Citation,
  documents: EvidenceDocument[],
): { supported: boolean; headers: Citation[] } {
  const invoice = documents.find(
    (d) => d.id === invoiceEvidence.documentId && d.kind === 'invoice',
  );
  const delivery = documents.find(
    (d) => d.id === deliveryEvidence.documentId && d.kind === 'delivery_note',
  );
  ...
  const billed =
    quantitySupported(invoiceEvidence.quote, item.invoicedQuantity, item.unit) ||
    billedTable?.quantity === item.invoicedQuantity;
  const received =
    quantitySupported(deliveryEvidence.quote, item.receivedQuantity, item.unit) ||
    receivedTable?.quantity === item.receivedQuantity;
  const price =
    labeledMoneySupported(invoiceEvidence.quote, item.unitPriceCents, 'price') ||
    billedTable?.price === item.unitPriceCents;
...
function grounded(
  ...
  return (
    ...
    citation.quote.trim().length >= 3 &&
    document.text.includes(citation.quote)
  );
}
function sameProduct(
  ...
  supplier?: Supplier | null,
  documents: EvidenceDocument[] = [],
): { matches: boolean; memory: boolean } {
  const observed = observedProductLabel(
    deliveryEvidence.quote,
    documents.find((document) => document.id === deliveryEvidence.documentId),
  );
  if (productKey(observed) === productKey(product)) return { matches: true, memory: false };
  for (const alias of supplier?.aliases ?? []) {
    const parts = alias.split(/\s*(?:=|→|↔)\s*/).filter(Boolean);
    if (
      parts.length === 2 &&
      parts.some((p) => productKey(p) === productKey(product)) &&
      parts.some((p) => productKey(observed) === productKey(p))
    )
      return { matches: true, memory: true };
  }
  // A supplied mapping is evidence, not permission to invent product equivalence.
  const equivalence = new RegExp(
    `^(?:Our\\s+invoice\\s+product\\s+)?${escapeRegExp(product)}\\s+(?:is described as|is also known as|is called|=)\\s+${escapeRegExp(observed)}(?:\\s+on delivery notes?)?[.!]?$`,
    'i',
  );
  if (
    documents.some(
      (document) =>
        document.kind === 'supplier_message' &&
        document.text.split('\n').some((line) => equivalence.test(line.trim())),
    )
  )
    return { matches: true, memory: false };
...
async function requestJson(
  ...
  try {
    ...
    const response = await fetch(url, {
      method: 'POST',
      headers: { ...auth, 'Content-Type': 'application/json' } as Record<string, string>,
    ...
    if (!response.ok) {
      ...
      if (response.status === 429)
        throw new EngineError(
          'The AI provider has reached its usage limit. Retry later or check API quota.',
          'AI_RATE_LIMITED',
          503,
        );
      if (response.status === 413)
        throw new EngineError(
          'The AI provider rejected an oversized request. Reduce document size or configure another provider.',
          'AI_REQUEST_LIMIT',
          422,
        );
      if (response.status === 404)
        throw new EngineError(
          'The configured AI model is unavailable for this account. Check the model setting.',
      ...
      if (response.status === 400) {
        ...
        throw new EngineError(
          'The AI provider rejected the request format. Check provider configuration.',
          'AI_REQUEST_REJECTED',
          502,
        );
      }
      throw new EngineError(
        'The AI provider is unavailable. Your documents and previous analysis are unchanged.',
        'AI_UNAVAILABLE',
        503,
      );
    }
    ...
    try {
      ...
      throw new EngineError(
        'AI returned an unreadable response. Retry analysis.',
        'AI_INVALID_OUTPUT',
        502,
      );
    }
    return {
      data,
      trace: response.headers.get('x-request-id') ?? response.headers.get('x-trace-id') ?? '',
    };
  } catch (error) {
    if (error instanceof EngineError) throw error;
    if (error instanceof Error && (error.name === 'TimeoutError' || error.name === 'AbortError'))
      throw new EngineError(
        'AI analysis timed out. Your documents and previous analysis are unchanged; try again.',
        'AI_TIMEOUT',
        504,
      );
    throw new EngineError(
      'Could not reach the AI provider. Your documents are saved; try again.',
      'AI_UNAVAILABLE',
      503,
    );
  }
...
function parseExtraction(value: unknown): Extraction {
  ...
  if (typeof value === 'string') {
    ...
    try {
      ...
      throw new EngineError(
        'AI did not return valid structured evidence. Retry analysis.',
        'AI_INVALID_OUTPUT',
        502,
      );
    }
  ...
  if (!result.success)
    throw new EngineError(
      'AI evidence failed validation. No financial amounts were changed. Retry analysis.',
      'AI_INVALID_OUTPUT',
      502,
    );
  return result.data;
...
const compactFinding = z.tuple([
  ...
  quantitySchema,
  quantitySchema,
  z.number().int().positive().max(MAX_MONEY),
  z.string().min(1).max(40),
  z.string().min(1),
  z.string().min(1),
]);
const compactComparison = z
  .object({ r: z.string().max(100), f: z.array(compactFinding).max(80) })
  .strict();
const compactCredit = z
  .object({
    k: z
      .array(
        z.tuple([
...
function parseCompact<T>(response: unknown, schema: z.ZodType<T>): T {
  ...
  if (typeof value === 'string') {
    try {
      ...
      throw new EngineError(
        'Evorozen did not return structured evidence. Retry analysis.',
        'AI_INVALID_OUTPUT',
        502,
      );
    }
  ...
  if (!parsed.success)
    throw new EngineError(
      'Evorozen evidence failed validation. No amounts were changed.',
      'AI_INVALID_OUTPUT',
      502,
    );
  return parsed.data;
...
async function evorozenExtraction(
  input: AnalyzeInput,
): Promise<{ extraction: Extraction; traceId: string; calls: number }> {
  const indexed = new Map<string, { document: EvidenceDocument; quote: string }>();
  const lines = (document: EvidenceDocument, prefix: string): string[] =>
    document.text
      .split('\n')
      .filter((line) => line.trim())
      .map((quote, index) => {
        const key = `${prefix}${index}`;
        indexed.set(key, { document, quote });
        return `${key} ${quote}`;
      });
  const invoice = input.documents.find((d) => d.kind === 'invoice')!;
  const delivery = input.documents.find((d) => d.kind === 'delivery_note')!;
  const invoiceLines = lines(invoice, 'I');
  const deliveryLines = lines(delivery, 'D');
  const memory = JSON.stringify({
    supplier: input.supplier?.name ?? '',
    aliases: input.supplier?.aliases ?? [],
    notes: input.supplier?.notes ?? '',
    messages: input.documents.filter((d) => d.kind === 'supplier_message').map((d) => d.text),
  });
  ...
  if (maxWindow < 200)
    throw new EngineError(
      'Supplier notes exceed Evorozen’s compact request limit. Shorten the notes or configure another provider for larger documents.',
      'AI_DOCUMENT_LIMIT',
      422,
    );
  const windows = (sourceLines: string[], max: number): string[] => {
    ...
    for (const line of sourceLines) {
      if (line.length > max)
        throw new EngineError(
          'A source line exceeds Evorozen’s compact request limit. Use clearer extracted rows or configure another provider for larger documents.',
          'AI_DOCUMENT_LIMIT',
          422,
        );
  ...
  const invoiceWindows = windows(invoiceLines, maxWindow);
  const deliveryWindows = windows(deliveryLines, maxWindow);
  const credits = input.documents.filter((d) => d.kind === 'credit_note');
  const creditWindows = credits.map((document, index) => ({
    document,
    windows: windows(lines(document, `C${index}_`), 1200),
  }));
  const callsNeeded =
    invoiceWindows.length * deliveryWindows.length +
    creditWindows.reduce((n, item) => n + item.windows.length, 0);
  const maxCalls = Math.max(
    1,
    Math.min(Number(process.env.EVOROZEN_MAX_CALLS_PER_ANALYSIS) || 8, 12),
  );
  if (callsNeeded > maxCalls)
    throw new EngineError(
      `These documents need ${callsNeeded} compact Evorozen requests, above this installation’s ${maxCalls}-call limit. Use a smaller consolidated record or configure another provider. No source text was truncated.`,
      'AI_DOCUMENT_LIMIT',
      422,
    );
  const findings: Extraction['findings'] = [];
  const matches: Extraction['credits'] = [];
  const traces: string[] = [];
  let reference = input.invoiceReference?.trim() ?? '';
  const request = async (prompt: string) => {
    if (prompt.length > 2000)
      throw new EngineError(
        'The evidence window exceeds the provider request limit.',
        'AI_DOCUMENT_LIMIT',
        422,
      );
    await input.beforeProviderRequest?.();
    const result = await requestJson(
      'https://pulse.evorozen.com/api/neural',
  ...
  for (const invoiceWindow of invoiceWindows)
    for (const deliveryWindow of deliveryWindows) {
      const result = parseCompact(
        await request(`${instruction}${invoiceWindow}\n${deliveryWindow}`),
        compactComparison,
      );
      if (result.r && hasReference(invoice.text, result.r)) {
        if (reference && normalized(reference) !== normalized(result.r))
          throw new EngineError(
            'Evorozen found conflicting invoice references. Use one invoice per case.',
            'INVOICE_MISMATCH',
          );
        reference = result.r;
      }
      for (const [
        ...
        const billed = indexed.get(invoiceId);
        const received = indexed.get(deliveryId);
        if (
          !billed ||
          !received ||
          billed.document.id !== invoice.id ||
          received.document.id !== delivery.id ||
          !invoiceWindow.split('\n').some((line) => line.startsWith(`${invoiceId} `)) ||
          !deliveryWindow.split('\n').some((line) => line.startsWith(`${deliveryId} `))
        )
          throw new EngineError(
            'Evorozen referenced evidence outside its supplied window.',
            'AI_INVALID_OUTPUT',
            502,
          );
        findings.push({
          ...
          confidence: 'high',
          explanation:
            'The AI matched the invoice item to its receiving record. The application independently validates quoted quantities, units and prices before calculating a proposed shortage.',
          invoiceEvidence: { documentId: invoice.id, quote: billed.quote },
          deliveryEvidence: { documentId: delivery.id, quote: received.quote },
        });
      }
    }
  if (!reference)
    throw new EngineError(
      'An invoice reference could not be grounded. Add the reference to the case or a clearer invoice header.',
      'UNGROUNDED_INVOICE',
    );
  for (const entry of creditWindows)
    for (const window of entry.windows) {
      ...
      for (const [creditReference, invoiceReference, currency, amountCents, ids] of result.k) {
        const evidence = ids.map((id) => {
          ...
          if (
            !source ||
            source.document.id !== entry.document.id ||
            !window.split('\n').some((line) => line.startsWith(`${id} `))
          )
            throw new EngineError(
              'Evorozen referenced a credit source outside its supplied window.',
              'AI_INVALID_OUTPUT',
              502,
            );
  ...
  return {
    ...
    traceId: traces.at(-1) || `local-${randomUUID()}`,
    calls: callsNeeded,
  };
}

async function liveExtraction(input: AnalyzeInput): Promise<{
  extraction: Extraction;
  provider: 'evorozen' | 'openai' | 'gemini';
  traceId: string;
  warnings: string[];
}> {
  const data = JSON.stringify({
    ...
    supplier: input.supplier
      ? { name: input.supplier.name, aliases: input.supplier.aliases, notes: input.supplier.notes }
      : null,
    documents: input.documents.map((d) => ({ id: d.id, kind: d.kind, text: d.text })),
  });
  const instructions = providerInstructions();
  const warnings: string[] = [];
  const selection = process.env.AI_PROVIDER || 'auto';
  if (!['auto', 'evorozen', 'openai', 'gemini'].includes(selection))
    throw new EngineError(
      'AI_PROVIDER must be auto, evorozen, openai or gemini.',
      'AI_CONFIGURATION',
      503,
    );
  const preferred =
  ...
  if (!key)
    throw new EngineError(
      'The selected AI provider has no server API key. Configure the selected provider or use the labeled demo workspace.',
      'AI_NOT_CONFIGURED',
      503,
    );
  const useEvorozen = preferred === 'evorozen';
  const useOpenAI =
    preferred === 'openai' || (useEvorozen && process.env.OPENAI_FALLBACK_ENABLED === 'true');
  const useGemini =
    preferred === 'gemini' ||
    ((useEvorozen || useOpenAI) && process.env.GEMINI_FALLBACK_ENABLED === 'true');
  if (useEvorozen) {
    try {
      const result = await evorozenExtraction(input);
      warnings.push(
        `Evorozen analyzed indexed source windows in ${result.calls} bounded request${result.calls === 1 ? '' : 's'}. All source lines were included; only grounded item matches affect amounts.`,
      );
      return {
      ...
      const allowOpenAI =
        process.env.OPENAI_API_KEY && process.env.OPENAI_FALLBACK_ENABLED === 'true';
      const allowGemini =
        process.env.GEMINI_API_KEY && process.env.GEMINI_FALLBACK_ENABLED === 'true';
      if (!allowOpenAI && !allowGemini) throw error;
      warnings.push(
        'Evorozen was unavailable or returned invalid evidence. Analysis used an explicitly enabled fallback provider.',
      );
    }
  }
  if (useOpenAI && process.env.OPENAI_API_KEY) {
    ...
    try {
      ...
      if (!process.env.GEMINI_API_KEY || process.env.GEMINI_FALLBACK_ENABLED !== 'true')
        throw error;
      warnings.push(
        'OpenAI was unavailable or returned invalid evidence. Analysis used the explicitly enabled Gemini fallback.',
      );
    }
  }
  if (useGemini && process.env.GEMINI_API_KEY) {
    ...
    if (!/^[a-zA-Z0-9._-]{1,100}$/.test(model))
      throw new EngineError(
        'GEMINI_MODEL must be a valid model identifier.',
        'AI_CONFIGURATION',
        503,
      );
    await input.beforeProviderRequest?.();
    const result = await requestJson(
      ...
      process.env.GEMINI_API_KEY,
      {
        ...
        generationConfig: {
          ...
          responseJsonSchema: geminiSchema(z.toJSONSchema(extractionSchema)),
    ...
    return {
      ...
      traceId:
        result.trace ||
        (typeof envelope.responseId === 'string' ? envelope.responseId : `local-${randomUUID()}`),
      warnings,
    };
  }
  throw new EngineError(
    'Live AI is not configured. Set EVOROZEN_API_KEY, GEMINI_API_KEY or OPENAI_API_KEY on the server, or use the labeled demo workspace for sample documents.',
    'AI_NOT_CONFIGURED',
    503,
  );
}
...
function demoExtraction(documents: EvidenceDocument[]): Extraction {
  ...
  if (
    documents.some(
      (d) =>
        !known.some((sample) => sample.kind === d.kind && sample.text.trim() === d.text.trim()),
    )
  )
    throw new EngineError(
      'Demo replay supports only the provided fictional Northstar sample documents. Create a personal workspace with a configured AI provider for your own documents.',
      'DEMO_FIXTURE_REQUIRED',
    );
  const invoice = documents.find((d) => d.kind === 'invoice' && d.text.trim() === SAMPLE_INVOICE);
  const delivery = documents.find(
    (d) => d.kind === 'delivery_note' && d.text.trim() === SAMPLE_DELIVERY,
  );
  if (!invoice || !delivery)
    throw new EngineError(
      'Add the sample invoice and delivery note before running demo analysis.',
      'MISSING_DOCUMENTS',
    );
    ...
    findings: [
      {
        ...
        confidence: 'high',
        explanation:
          'The invoice bills twelve cases; the receiving record confirms eight. The supplier alias links the two product descriptions.',
        invoiceEvidence: { documentId: invoice.id, quote: line(SAMPLE_INVOICE, 'OAT BARISTA') },
        deliveryEvidence: { documentId: delivery.id, quote: line(SAMPLE_DELIVERY, 'barista oat') },
      },
      {
        product: 'TOMATO WHOLE 6X2.5KG',
        kind: 'shortage',
        invoicedQuantity: 10,
        receivedQuantity: 7,
        unitPriceCents: 2400,
        ...
        confidence: 'high',
        explanation:
          'Ten cases were billed and seven were received, leaving three cases for supplier review.',
        invoiceEvidence: { documentId: invoice.id, quote: line(SAMPLE_INVOICE, 'TOMATO') },
        deliveryEvidence: { documentId: delivery.id, quote: line(SAMPLE_DELIVERY, 'TOMATO') },
      },
    ],
    credits: credit
      ? [
...
export async function analyzeDocuments(input: AnalyzeInput): Promise<Analysis> {
  ...
  if (input.supplier && input.rememberedAliases?.length) {
    input = {
      ...
      supplier: {
        ...
        aliases: [
          ...new Set([
            ...input.supplier.aliases,
            ...input.rememberedAliases
              .filter((value) => typeof value === 'string' && value.length <= 405)
              .slice(0, 30),
          ]),
        ],
      },
  ...
  if (
    ...
    input.documents.some((d) => !d.text.trim() || d.text.length > 40_000)
  )
    throw new EngineError(
      'Provide between 1 and 12 readable documents, each under 40,000 characters.',
      'INVALID_DOCUMENTS',
    );
  if (
    !input.documents.some((d) => d.kind === 'invoice') ||
    !input.documents.some((d) => d.kind === 'delivery_note')
  )
    throw new EngineError(
      'Add an invoice and delivery note to compare billed and received quantities.',
      'MISSING_DOCUMENTS',
    );
  if (
    input.documents.filter((d) => d.kind === 'invoice').length !== 1 ||
    input.documents.filter((d) => d.kind === 'delivery_note').length !== 1
  )
    throw new EngineError(
      'Use one invoice and one consolidated receiving record per case. Multiple invoices or partial receiving notes can double-count shortages.',
      'AMBIGUOUS_DOCUMENT_SET',
    );
  const result = input.isDemo
    ? {
        extraction: demoExtraction(input.documents),
        provider: 'demo' as const,
        traceId: `demo-${analysisSourceHash(input.documents, input.supplier).slice(0, 16)}`,
        warnings: ['Fictional sample replay. No live AI request was made.'],
      }
    : await liveExtraction(input);
  const raw = result.extraction;
  if (
    input.invoiceReference?.trim() &&
    normalized(input.invoiceReference) !== normalized(raw.invoiceReference)
  )
    throw new EngineError(
      'The document invoice reference differs from this case. Correct the case reference or use a separate case.',
      'INVOICE_MISMATCH',
    );
  if (
    raw.currency !== input.currency ||
    input.documents.some((d) => currencyIn(d.text).some((code) => code !== input.currency))
  )
    throw new EngineError(
      'A document currency differs from this workspace. Use a separate workspace for each currency; no exchange conversion was applied.',
      'CURRENCY_MISMATCH',
    );
  const invoice = input.documents.find(
    (d) => d.kind === 'invoice' && hasInvoiceReference(d.text, raw.invoiceReference),
  );
  if (!invoice)
    throw new EngineError(
      'The extracted invoice reference is missing from the supplied invoice. Correct the source and retry.',
      'UNGROUNDED_INVOICE',
    );
  const supplierNames = input.supplier
    ? [input.supplier.name, ...input.supplier.aliases]
    : [raw.supplierName];
  const invoiceSupplier = sourceSupplierHeader(invoice.text, supplierNames);
  if (!invoiceSupplier)
    throw new EngineError(
      'The invoice header does not identify this case supplier. Correct the supplier or add a confirmed supplier-name alias and reanalyze.',
      'SUPPLIER_MISMATCH',
    );
  supplierNames.push(invoiceSupplier.name);
  const warnings = [...result.warnings];
  if (input.rememberedAliases?.length)
    warnings.push(
      `Loaded ${input.rememberedAliases.length} signed supplier alias mapping(s) from Evorozen memory. Trace: ${(input.memoryTraceId || 'unavailable').slice(0, 120)}. Source evidence remains required.`,
    );
  const suspicious =
    input.documents.some((d) => hasInjection(d.text)) ||
    hasInjection(input.supplier?.notes ?? '') ||
    (input.supplier?.aliases ?? []).some(hasInjection);
  if (suspicious)
    warnings.push(
      'Instruction-like content was detected in source data. Findings are blocked from approval; remove the unrelated instructions and reanalyze.',
    );
  const currencyGrounded = validateCurrency(invoice.text, input.currency);
  if (!currencyGrounded)
    warnings.push(
      'Invoice currency is not explicitly established. Add an unambiguous currency code and reanalyze.',
    );
  ...
  for (const item of raw.findings) {
    const financialEvidence = financialEvidenceSupported(
      item,
      item.invoiceEvidence,
      item.deliveryEvidence,
      input.documents,
    );
    const productMatch = sameProduct(
      item.product,
      item.deliveryEvidence,
      input.supplier,
      input.documents,
    ...
    const supported =
      grounded(item.invoiceEvidence, input.documents, 'invoice') &&
      grounded(item.deliveryEvidence, input.documents, 'delivery_note') &&
      productKey(
        observedProductLabel(
          item.invoiceEvidence.quote,
          input.documents.find((d) => d.id === item.invoiceEvidence.documentId),
        ),
      ) === productKey(item.product) &&
      productMatch.matches &&
      financialEvidence.supported &&
      item.invoicedQuantity > item.receivedQuantity &&
      hasInvoiceReference(
        input.documents.find((d) => d.id === item.invoiceEvidence.documentId)?.text ?? '',
        raw.invoiceReference,
      ) &&
      hasInvoiceReference(
        input.documents.find((d) => d.id === item.deliveryEvidence.documentId)?.text ?? '',
        raw.invoiceReference,
      ) &&
      !!sourceSupplierHeader(
        input.documents.find((d) => d.id === item.deliveryEvidence.documentId)?.text ?? '',
        supplierNames,
      );
    const needsReview =
    ...
    const amountCents =
      item.kind === 'shortage'
        ? shortageCents(item.invoicedQuantity, item.receivedQuantity, item.unitPriceCents)
        : 0;
    const identity = JSON.stringify([
      item.invoiceEvidence.documentId,
      item.invoiceEvidence.quote,
      item.deliveryEvidence.documentId,
      item.deliveryEvidence.quote,
    ]);
    const invoiceText =
      input.documents.find((d) => d.id === item.invoiceEvidence.documentId)?.text ?? '';
    ...
    const overlapping =
      start >= 0 &&
      usedInvoiceSpans.some(
        (span) =>
          span.documentId === item.invoiceEvidence.documentId &&
          start < span.end &&
          end > span.start,
      );
    if (fingerprints.has(identity) || overlapping) {
      warnings.push(
        'A duplicate proposed finding was discarded. Each source line can be recovered only once.',
      );
      continue;
    }
    fingerprints.add(identity);
    if (start >= 0)
      usedInvoiceSpans.push({ documentId: item.invoiceEvidence.documentId, start, end });
    ...
    findings.push({
      ...
      evidence: [item.invoiceEvidence, item.deliveryEvidence, ...financialEvidence.headers],
      accepted: !needsReview && !!previous?.accepted,
      needsReview,
    });
  }
  if (findings.some((f) => f.needsReview))
    warnings.push(
      'Some findings lack sufficient grounded support. They cannot be included in a claim until the evidence is corrected and reanalyzed.',
    );
  ...
  for (const credit of raw.credits) {
    const document = input.documents.find(
      (d) => d.id === credit.documentId && d.kind === 'credit_note',
    );
    ...
    const valid =
      document &&
      creditSupplier &&
      credit.invoiceReference === raw.invoiceReference &&
      credit.currency === input.currency &&
      validateCurrency(document.text, input.currency) &&
      credit.evidence.every(
        (c) => c.documentId === credit.documentId && grounded(c, input.documents, 'credit_note'),
      ) &&
      hasCreditReference(allQuotes, credit.reference) &&
      hasInvoiceReference(allQuotes, raw.invoiceReference) &&
      labeledMoneySupported(allQuotes, credit.amountCents, 'credit');
    if (!valid || suspicious) {
      warnings.push(
        'A proposed credit did not have an exact supplier, invoice, currency, amount and source match; it was excluded.',
      );
      continue;
    }
    const key = normalized(credit.reference);
    if (creditRefs.has(key)) {
      warnings.push('A duplicate credit reference was excluded.');
      continue;
    }
    creditRefs.add(key);
    const previous = input.previousAnalysis?.credits.find(
      (c) =>
        c.documentId === credit.documentId &&
        c.reference === credit.reference &&
        c.amountCents === credit.amountCents &&
        c.verified,
    );
    const evidence = credit.evidence.some((c) => c.quote.includes(creditSupplier!.quote))
      ? credit.evidence
      : [{ documentId: credit.documentId, quote: creditSupplier!.quote }, ...credit.evidence];
    credits.push({
  ...
  return {
    ...
    supplierName: invoiceSupplier.name,
    invoiceReference: raw.invoiceReference,
    currency: input.currency,
    provider: result.provider,
    traceId: result.traceId.slice(0, 200),
    durationMs: Date.now() - started,
    analyzedAt: new Date().toISOString(),
    sourceHash: analysisSourceHash(input.documents, originalSupplier),
    memoryUsed,
  };
}
...
export function computeTotals(
  ...
  if (
    safe.some(
      (f) =>
        f.kind !== 'shortage' ||
        f.amountCents !== shortageCents(f.invoicedQuantity, f.receivedQuantity, f.unitPriceCents),
    )
  )
    throw new EngineError(
      'A finding amount does not match the documented calculation.',
      'INVALID_TOTALS',
    );
  const identifiedCents = safe.reduce((sum, f) => sum + f.amountCents, 0);
  const claimedCents =
    approvedClaimedCents ??
    safe.filter((f) => f.accepted).reduce((sum, f) => sum + f.amountCents, 0);
  ...
  const creditedCents = credits
    ...
    .reduce((sum, credit) => {
      const key = normalized(credit.reference);
      if (refs.has(key))
        throw new EngineError('Duplicate verified credit reference.', 'DUPLICATE_CREDIT');
      refs.add(key);
      return sum + credit.amountCents;
    }, 0);
  if (
    [identifiedCents, claimedCents, creditedCents].some((n) => !Number.isSafeInteger(n) || n < 0) ||
    creditedCents > claimedCents
  )
    throw new EngineError(
      'Credit totals exceed the reviewed claim or contain invalid money.',
      'INVALID_TOTALS',
    );
  return {
    identifiedCents,
    claimedCents,
    creditedCents,
    remainingCents: claimedCents - creditedCents,
  };
}
...
export function creditSourceSupplierKey(credit: CreditMatch, recoveryCase: RecoveryCase): string {
  const document = recoveryCase.documents.find(
    (d) => d.id === credit.documentId && d.kind === 'credit_note',
  );
  const issuer = document
    ? sourceSupplierHeader(document.text, [
        recoveryCase.supplierName,
        recoveryCase.analysis?.supplierName ?? '',
      ])
    : null;
  if (
    !issuer ||
    !credit.evidence.some(
      (c) =>
        c.documentId === credit.documentId &&
        grounded(c, recoveryCase.documents, 'credit_note') &&
        c.quote.includes(issuer.quote),
    )
  )
    throw new EngineError(
      'The credit note does not quote the same supplier as the invoice.',
      'SUPPLIER_MISMATCH',
    );
...
export function verifyCreditMatch(credit: CreditMatch, recoveryCase: RecoveryCase): CreditMatch {
  ...
  if (
    ...
    verified.some(
      (c) =>
        c.documentId === credit.documentId ||
        normalized(c.reference) === normalized(credit.reference),
    )
  )
    throw new EngineError('This credit has already been verified.', 'DUPLICATE_CREDIT', 409);
  creditSourceSupplierKey(credit, recoveryCase);
  const text = credit.evidence.map((c) => c.quote).join('\n');
  if (
    !document ||
    !credit.evidence.length ||
    credit.evidence.some(
      (c) =>
        c.documentId !== credit.documentId || !grounded(c, recoveryCase.documents, 'credit_note'),
    ) ||
    !hasCreditReference(text, credit.reference) ||
    !hasInvoiceReference(text, recoveryCase.invoiceReference) ||
    !labeledMoneySupported(text, credit.amountCents, 'credit') ||
    !validateCurrency(document.text, recoveryCase.currency) ||
    hasInjection(document.text)
  )
    throw new EngineError(
      'The credit needs exact source evidence for its reference, invoice, currency and total.',
      'UNGROUNDED_CREDIT',
    );
  if (
    !Number.isSafeInteger(credit.amountCents) ||
    credit.amountCents <= 0 ||
    credit.amountCents >
      recoveryCase.claimedCents - verified.reduce((sum, c) => sum + c.amountCents, 0)
  )
    throw new EngineError(
      'The credit is larger than the outstanding reviewed claim.',
      'CREDIT_EXCEEDS_CLAIM',
    );
  if (!['approved', 'sent', 'partial'].includes(recoveryCase.status) || !recoveryCase.claimedCents)
    throw new EngineError(
      'Prepare a reviewed claim before verifying a credit.',
      'CLAIM_REQUIRED',
      409,
    );
...
export function buildClaim(recoveryCase: RecoveryCase, workspaceName: string): string {
  if (!recoveryCase.analysis)
    throw new EngineError('Analyze the documents before preparing a claim.', 'ANALYSIS_REQUIRED');
  const accepted = recoveryCase.analysis.findings.filter((f) => f.accepted);
  if (!accepted.length)
    throw new EngineError(
      'Review and select at least one supported finding.',
      'NO_ACCEPTED_FINDINGS',
    );
  const supplierNames = [recoveryCase.supplierName, recoveryCase.analysis.supplierName];
  if (
    recoveryCase.documents
      .filter((d) => d.kind === 'invoice' || d.kind === 'delivery_note')
      .some(
        (d) =>
          !sourceSupplierHeader(d.text, supplierNames) ||
          !hasInvoiceReference(d.text, recoveryCase.invoiceReference),
      )
  )
    throw new EngineError(
      'The invoice and receiving record must identify the same supplier and invoice as this case.',
      'SUPPLIER_MISMATCH',
    );
  if (
    accepted.some((f) => {
      const invoiceEvidence = f.evidence.find((c) =>
        grounded(c, recoveryCase.documents, 'invoice'),
      );
      const deliveryEvidence = f.evidence.find((c) =>
        grounded(c, recoveryCase.documents, 'delivery_note'),
      );
      return (
        f.needsReview ||
        f.confidence === 'low' ||
        ...
        !invoiceEvidence ||
        !deliveryEvidence ||
        !financialEvidenceSupported(f, invoiceEvidence, deliveryEvidence, recoveryCase.documents)
          .supported ||
        f.evidence.some((c) => !grounded(c, recoveryCase.documents)) ||
        f.amountCents !== shortageCents(f.invoicedQuantity, f.receivedQuantity, f.unitPriceCents)
      );
    }) ||
    recoveryCase.documents.some(
      (d) =>
        hasInjection(d.text) || currencyIn(d.text).some((code) => code !== recoveryCase.currency),
    )
  )
    throw new EngineError(
      'A selected finding lacks verified source support. Correct the evidence and reanalyze.',
      'UNVERIFIED_FINDINGS',
    );
