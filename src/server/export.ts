...
export function caseCsv(value: RecoveryCase): string {
  ...
  const rows = (value.analysis?.findings ?? []).map((finding) => [
    value.id,
    value.invoiceReference,
    value.currency,
    finding.product,
    finding.kind,
    finding.invoicedQuantity,
    finding.receivedQuantity,
    finding.unit,
    finding.unitPriceCents,
    finding.amountCents,
    finding.accepted,
    finding.needsReview,
    finding.evidence.map((citation) => `${citation.documentId}: ${citation.quote}`).join(' | '),
  ]);
  return '\uFEFF' + [headers, ...rows].map((row) => row.map(cell).join(',')).join('\r\n') + '\r\n';
}
/** Keep the original approved request immutable; derive later correspondence from the ledger. */
export function currentCorrespondence(
  value: RecoveryCase,
  ...
  return {
    ...
    body:
      value.claimText.replace(/^Subject:[^\r\n]*\r?\n\s*\r?\n/i, '') ||
      `Please review the attached evidence for ${reference}.\n\n${workspaceName}`,
  };
}
...
export async function casePdf(value: RecoveryCase, user: User): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      bufferPages: true,
      info: {
        Title: `Remainder evidence pack - ${value.invoiceReference || value.title}`,
        Author: user.workspaceName,
        Subject: 'Supplier review request with source evidence',
      },
    });
    ...
    if (value.claimText) {
      section(
        value.creditedCents > 0
          ? 'Original approved request (historical record)'
          : 'Supplier review request',
      );
      if (value.creditedCents > 0)
        text(
          'The request below is retained for the audit trail. Issued credits are deducted in the current recovery position above.',
        );
    ...
    for (const [index, finding] of (value.analysis?.findings ?? []).entries()) {
      ...
      doc
        ...
        .fontSize(11)
        .text(
          `${index + 1}. ${finding.product} - ${formatMoney(finding.amountCents, value.currency)}`,
        );
      text(
        `${finding.kind} | ${finding.accepted ? 'Included' : 'Excluded'} | Confidence: ${finding.confidence}${finding.needsReview ? ' | REQUIRES REVIEW' : ''}\n${finding.explanation}`,
      );
      text(
        `Invoice quantity ${finding.invoicedQuantity}; delivered ${finding.receivedQuantity}; unit ${finding.unit}; unit price ${formatMoney(finding.unitPriceCents, value.currency)}.`,
      );
    ...
    if (value.analysis?.credits.length) {
      ...
      for (const credit of value.analysis.credits)
        text(
          `${credit.reference}: ${formatMoney(credit.amountCents, value.currency)} - ${credit.verified ? 'verified' : 'pending verification'}\n${credit.evidence.map((citation) => `"${citation.quote}"`).join('\n')}`,
        );
    }
    ...
    text(
      'This pack requests supplier review of submitted evidence. It is not a legal, tax, or accounting determination. The workspace owner approves findings before requesting a credit.',
    );
    ...
    for (let index = pages.start; index < pages.start + pages.count; index++) {
      ...
      doc
        ...
        .fillColor('#63736b')
        .text(
          `Remainder | ${value.invoiceReference || 'Evidence pack'} | ${index + 1} / ${pages.count}`,
          48,
          790,
          { lineBreak: false },
        );
