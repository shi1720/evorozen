...
test('settings export and password-confirmed account deletion work end to end', async ({
  page,
}) => {
  ...
  const registered = await page.request.post('/api/auth/register', {
    data: {
      ...
      email,
      password,
      workspaceName: 'Export First Cafe',
      currency: 'USD',
    },
  });
  expect(registered.status()).toBe(201);
  const created = await page.request.post('/api/cases', {
    data: { title: 'One private case', supplierName: 'Meadow Foods' },
  });
  ...
  await page.screenshot({
    path: 'output/screenshots/settings-delete-confirmation.png',
    fullPage: true,
  });
  await destroy.click();
  await expect(page).toHaveURL('/');
  expect((await page.request.get('/api/auth/me')).status()).toBe(401);
  expect((await page.request.post('/api/auth/login', { data: { email, password } })).status()).toBe(
    401,
  );
});
